...
class ResponceStoriesSJ: AnyStoryResponseProtocol {
    var code: Int?
    var status: String?
    ...
    convenience init(from json: JSON) {
        ...
        self.data = DataStoriesSJ(from: json["data"])
    }
}

class DataStoriesSJ: AnyStoryDataProtocol {
    var offset: Int?
    var limit: Int?
    var total: Int?
    var count: Int?
    ...
    convenience init(from json: JSON) {
        ...
        self.results = resultsJSONs.map { StoriesSJ (from: $0) }
    }
}


class StoriesSJ: Object, AnyStoryProtocol {
