...
class ResponceEventSJ: AnyStoryResponseProtocol {
    var code: Int?
    var status: String?
    ...
    convenience init(from json: JSON) {
        ...
        self.data = DataEventSJ(from: json["data"])
    }
}

class DataEventSJ: AnyStoryDataProtocol {
    var offset: Int?
    var limit: Int?
    var total: Int?
    var count: Int?
    ...
    convenience init(from json: JSON) {
        ...
        self.results = resultsJSONs.map { EventsSJ (from: $0) }
    }
}


class EventsSJ: Object, AnyStoryProtocol {
